...
`K₊ˣ → Kˣ → P_K`. Its range is `P_K⁺`, the subgroup used in the narrow quotient. -/
...
/-- Principal fractional ideals with a totally positive generator. P_K⁺ -/
...
Which means Cl⁺(R) = I(R) / P⁺(R) where P⁺(R) = { (x) | x ∈ Rˣ and x is totally positive }.
-/
