...
/-- The principal ideal generated by a nonzero element, bundled as a nonzero
ideal. -/
def spanSingletonNonzero {R : Type*} [CommRing R] [IsDomain R] [IsDedekindDomain R]
    {x : R} (hx : x ≠ 0) :
    (Ideal R)⁰ :=
  ⟨span ({x} : Set R), by
    rw [mem_nonZeroDivisors_iff_ne_zero, Ideal.zero_eq_bot, ne_eq, span_singleton_eq_bot]
    exact hx⟩

@[simp]
theorem coe_spanSingletonNonzero {R : Type*} [CommRing R] [IsDomain R]
    [IsDedekindDomain R] {x : R} (hx : x ≠ 0) :
    (spanSingletonNonzero hx : Ideal R) = span ({x} : Set R) :=
  rfl
...
theorem comap_mul_of_ringEquiv {R S : Type*} [CommRing R] [CommRing S]
  ...
  change (I * J).comap e = I.comap e * J.comap e
  rw [← Ideal.map_symm (f := e) (I := I * J),
    ← Ideal.map_symm (f := e) (I := I), ← Ideal.map_symm (f := e) (I := J), Ideal.map_mul]
