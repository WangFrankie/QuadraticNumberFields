...
/-- The category structure on bundled quadratic fields, with morphisms the
`ℚ`-algebra homomorphisms. -/
...
/-- Applying the algebra equivalence extracted from an isomorphism is applying
the isomorphism's forward morphism. -/
@[simp]
theorem algEquivOfIso_apply {K L : QuadraticFieldCat.{u}} (i : K ≅ L) (x : K) :
    algEquivOfIso i x = (show K →ₐ[ℚ] L from i.hom) x :=
  rfl

/-- Applying the inverse of the algebra equivalence extracted from an
isomorphism is applying the isomorphism's inverse morphism. -/
@[simp]
