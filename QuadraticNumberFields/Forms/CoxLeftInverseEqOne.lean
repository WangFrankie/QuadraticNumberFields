...
theorem classGroupToFormClass_idealClassOfForm_leftInverse_of_mod_four_eq_one
  ...
  rw [show idealClassOfForm_of_mod_four_eq_one d hd4 Q = ClassGroup.mk0 I from rfl,
    classGroupToFormClass_mk0_eq_formClassOfNonzeroIdeal hdneg I]
  rcases coxIdealBasisOKEqOne_K_re_im hd4 hdneg Q with ⟨h0_re, h0_im, h1_re, h1_im⟩
  have ha_pos_int : 0 < Q.1.a := Q.2.2.2.1
  have ha_pos : 0 < (Q.1.a : ℚ) := by exact_mod_cast ha_pos_int
  -- The transported Cox basis, oriented because `a > 0` and the `β`-coordinate `-1/2 < 0`.
  have hpos : imPartRatio (((coxIdealBasisOKEqOne hd4 hdneg Q 0 : 𝓞K) : K) *
        star ((coxIdealBasisOKEqOne hd4 hdneg Q 1 : 𝓞K) : K) -
        ((coxIdealBasisOKEqOne hd4 hdneg Q 1 : 𝓞K) : K) *
        star ((coxIdealBasisOKEqOne hd4 hdneg Q 0 : 𝓞K) : K)) > 0 := by
    rw [imPartRatio_wedge, h0_re, h0_im, h1_re, h1_im]; nlinarith [ha_pos]
  let b : OrientedBasis (I : Ideal 𝓞K) :=
    { basis := coxIdealBasisOKEqOne hd4 hdneg Q, oriented := hpos }
  -- `N(I) = a`, computed from the determinant against the `d % 4 = 1` integral basis
  -- (whose covolume contributes the extra factor of `2`, matching `im_val = -1/2`).
  have hdetCoord : b.detCoord = -(Q.1.a : ℚ) / 2 := by
    change ((coxIdealBasisOKEqOne hd4 hdneg Q 0 : 𝓞K) : K).re *
          ((coxIdealBasisOKEqOne hd4 hdneg Q 1 : 𝓞K) : K).im -
        ((coxIdealBasisOKEqOne hd4 hdneg Q 0 : 𝓞K) : K).im *
          ((coxIdealBasisOKEqOne hd4 hdneg Q 1 : 𝓞K) : K).re = _
    rw [h0_re, h0_im, h1_re, h1_im]; ring
  have hN_eq : (Ideal.absNorm (I : Ideal 𝓞K) : ℤ) = Q.1.a := by
    have hz_eq : ((RingOfIntegers.ringOfIntegersBasisOfModFourEqOne hd4).det
      ...
      have hz_cast : (((RingOfIntegers.ringOfIntegersBasisOfModFourEqOne hd4).det
          ((↑) ∘ b.basis) : ℤ) : ℚ) = -(Q.1.a : ℚ) := by
        rw [b.det_eq_one_cast_eq_two_detCoord hd4, hdetCoord]; ring
      exact_mod_cast hz_cast
    have hz_abs := b.det_eq_one_natAbs_eq_absNorm hd4
    rw [hz_eq] at hz_abs
    simp only [Int.natAbs_neg] at hz_abs
    omega
  -- The norm relation `(b/2)² − d·(-1/2)² = a·c`, from the field discriminant `b² − 4ac = d`.
  have hac : (Q.1.b / 2 : ℚ) ^ 2 - (d : ℚ) * (-1 / 2 : ℚ) ^ 2 = (Q.1.a : ℚ) * Q.1.c := by
  ...
  exact formClassOfNonzeroIdeal_eq_mk_of_oriented hdneg I Q b (-1 / 2)
    h0_re h0_im h1_re h1_im hac hN_eq
