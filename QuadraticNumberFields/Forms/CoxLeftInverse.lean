...
theorem classGroupToFormClass_idealClassOfForm_leftInverse_of_mod_four_ne_one
  ...
  rw [show idealClassOfForm_of_mod_four_ne_one d hd4 Q = ClassGroup.mk0 I from rfl,
  ...
  -- The transported Cox basis, oriented because `a > 0` and the `β`-coordinate `-1 < 0`.
  have hpos : imPartRatio (((coxIdealBasisOK hd4 hdneg Q 0 : 𝓞K) : K) *
        star ((coxIdealBasisOK hd4 hdneg Q 1 : 𝓞K) : K) -
        ((coxIdealBasisOK hd4 hdneg Q 1 : 𝓞K) : K) *
        star ((coxIdealBasisOK hd4 hdneg Q 0 : 𝓞K) : K)) > 0 := by
    rw [imPartRatio_wedge, h0_re, h0_im, h1_re, h1_im]; nlinarith [ha_pos]
  let b : OrientedBasis (I : Ideal 𝓞K) := { basis := coxIdealBasisOK hd4 hdneg Q, oriented := hpos }
  -- `N(I) = a`, computed from the determinant against the `d % 4 ≠ 1` integral basis.
  have hdetCoord : b.detCoord = -(Q.1.a : ℚ) := by
    change ((coxIdealBasisOK hd4 hdneg Q 0 : 𝓞K) : K).re *
          ((coxIdealBasisOK hd4 hdneg Q 1 : 𝓞K) : K).im -
        ((coxIdealBasisOK hd4 hdneg Q 0 : 𝓞K) : K).im *
          ((coxIdealBasisOK hd4 hdneg Q 1 : 𝓞K) : K).re = _
  ...
  have hN_eq : (Ideal.absNorm (I : Ideal 𝓞K) : ℤ) = Q.1.a := by
    have hz_eq : ((RingOfIntegers.ringOfIntegersBasisOfModFourNeOne hd4).det
      ...
      have hz_cast : (((RingOfIntegers.ringOfIntegersBasisOfModFourNeOne hd4).det
        ...
        rw [b.det_ne_one_cast_eq_detCoord hd4, hdetCoord]
      exact_mod_cast hz_cast
    have hz_abs := b.det_ne_one_natAbs_eq_absNorm hd4
    rw [hz_eq] at hz_abs
    simp only [Int.natAbs_neg] at hz_abs
    omega
  -- The norm relation `(b/2)² − d·(-1)² = a·c`, from the field discriminant `b² − 4ac = 4d`.
  have hac : (Q.1.b / 2 : ℚ) ^ 2 - (d : ℚ) * (-1 : ℚ) ^ 2 = (Q.1.a : ℚ) * Q.1.c := by
  ...
  exact formClassOfNonzeroIdeal_eq_mk_of_oriented hdneg I Q b (-1) h0_re h0_im h1_re h1_im hac hN_eq
