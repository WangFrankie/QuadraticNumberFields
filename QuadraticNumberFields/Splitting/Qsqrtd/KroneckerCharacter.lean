...
of the Dirichlet character `kroneckerCharacter (D(d))`, alongside the existing
symbol-form statements in `Splitting/Qsqrtd/Kronecker.lean`.
...
* `disc_emod_four_eq_zero_or_one`: `D(d) % 4 ∈ {0, 1}` for any squarefree `d`.
* `isSplit_iff_kroneckerCharacter_disc_eq_one`: `(p)` splits ↔
  `kroneckerCharacter (D(d)) ((p : ℤ) : ZMod (D(d)).natAbs) = 1`.
* `isInert_iff_kroneckerCharacter_disc_eq_neg_one`: inert ↔ value `-1`.
* `isRamified_iff_kroneckerCharacter_disc_eq_zero`: ramified ↔ value `0`.
* `isSplit_iff_kroneckerCharacter_discr_eq_one` (and friends): the same
  statements with `D(d)` replaced by `NumberField.discr (ℚ(√d))`.
-/
...
-- `𝓞(d)` and `𝔭(p)` are shared scoped notation; mirror `Splitting/Qsqrtd/Kronecker.lean`.
local notation3 "D(" d ")" => if d % 4 = 1 then d else 4 * d

/-! ## Auto-discharge instance for the discriminant `Fact` -/

/-- The field discriminant of a quadratic field has residue `0` or `1` modulo `4`. -/
lemma disc_emod_four_eq_zero_or_one (d : ℤ) [Fact (Squarefree d)] :
...
/-- Auto-discharged `Fact` for any quadratic-field discriminant. -/
instance (d : ℤ) [Fact (Squarefree d)] :
    Fact ((if d % 4 = 1 then d else 4 * d) % 4 = 0 ∨
          (if d % 4 = 1 then d else 4 * d) % 4 = 1) :=
  ⟨disc_emod_four_eq_zero_or_one d⟩
...
instance (d : ℤ) [Fact (Squarefree d)] [Fact (d ≠ 1)] :
  ...
  ⟨by rw [RingOfIntegers.discr_formula d]; exact disc_emod_four_eq_zero_or_one d⟩

/-! ## Character-form splitting iff theorems -/

variable (d : ℤ) [Fact (Squarefree d)] [Fact (d ≠ 1)]

/-- `(p)` splits in `𝓞(ℚ(√d))` exactly when the Kronecker character of the field
discriminant evaluates to `1` at the prime `p`. -/
theorem isSplit_iff_kroneckerCharacter_disc_eq_one (p : ℕ) [Fact p.Prime] :
    Ideal.IsSplitIn (𝔭(p)) 𝓞(d) ↔
      kroneckerCharacter (D(d)) ((p : ℤ) : ZMod (D(d)).natAbs) = 1 := by
  rw [kroneckerCharacter_apply_intCast, kroneckerSym_natCast,
    isSplit_iff_kroneckerSymNat_disc_eq_one d p]

/-- `(p)` is inert in `𝓞(ℚ(√d))` exactly when the Kronecker character of the field
discriminant evaluates to `-1` at the prime `p`. -/
theorem isInert_iff_kroneckerCharacter_disc_eq_neg_one (p : ℕ) [Fact p.Prime] :
    Ideal.IsInertIn (𝔭(p)) 𝓞(d) ↔
      kroneckerCharacter (D(d)) ((p : ℤ) : ZMod (D(d)).natAbs) = -1 := by
  rw [kroneckerCharacter_apply_intCast, kroneckerSym_natCast,
    isInert_iff_kroneckerSymNat_disc_eq_neg_one d p]

/-- `(p)` ramifies in `𝓞(ℚ(√d))` exactly when the Kronecker character of the field
discriminant evaluates to `0` at the prime `p`. -/
theorem isRamified_iff_kroneckerCharacter_disc_eq_zero (p : ℕ) [Fact p.Prime] :
    Ideal.IsRamifiedIn (𝔭(p)) 𝓞(d) ↔
      kroneckerCharacter (D(d)) ((p : ℤ) : ZMod (D(d)).natAbs) = 0 := by
  rw [kroneckerCharacter_apply_intCast, kroneckerSym_natCast,
    isRamified_iff_kroneckerSymNat_disc_eq_zero d p]
