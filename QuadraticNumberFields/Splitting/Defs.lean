...
section SingletonReduction

/-- When `p` has a unique prime above it, the uniform inertia degree reduces to the
per-prime inertia degree — no Galois hypothesis required, since the `Exists.choose`
...
lemma inertiaDegIn_eq_inertiaDeg_of_primesOver_eq_singleton {P : Ideal S}
  ...
  rw [hP, Set.mem_singleton_iff] at hchoose
  rw [hchoose]

/-- When `p` has a unique prime above it, the uniform ramification index reduces to the
...
This is the *only* step that uses the Galois construction (and hence `ringChar R ≠ 2`,
needed to make the quadratic extension separable, hence Galois). The trichotomy
`efg_trichotomy` below is pure arithmetic on top of this equality.

To generalise the whole splitting theory (e.g. to positive characteristic / inseparable
quadratic extensions), replace **only this lemma** with a proof from the non-Galois
`Ideal.sum_ramification_inertia` (`∑_{P∣p} e(P)·f(P) = [Frac S : Frac R]`) together with
the `*_of_primesOver_eq_singleton` uniformity bridges; `efg_trichotomy` and every
...
/-- For a degree-2 extension of Dedekind domains with `ringChar R ≠ 2`, the numerical
invariants `(g, e, f)` of a nonzero maximal prime fall into exactly one of three cases:
split `(2, 1, 1)`, inert `(1, 1, 2)`, or ramified `(1, 2, 1)`. Pure arithmetic on top of
`ncard_primesOver_mul_ramificationIdxIn_mul_inertiaDegIn_eq_two`. -/
