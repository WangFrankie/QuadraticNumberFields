...
/-- The ideal generated by a rational prime `p` is nonzero. -/
theorem pIdeal_ne_bot {p : ℕ} (hp : p.Prime) : (𝔭(p) : Ideal ℤ) ≠ ⊥ := by
  rw [Ne, Ideal.span_singleton_eq_bot, Nat.cast_eq_zero]
  exact hp.ne_zero

/-- The ideal generated by a rational prime `p` is maximal. -/
theorem pIdeal_isMaximal {p : ℕ} (hp : p.Prime) : (𝔭(p) : Ideal ℤ).IsMaximal :=
