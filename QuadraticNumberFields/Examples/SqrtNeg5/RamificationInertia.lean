...
noncomputable def phiPlus9 : R →+* ZMod 9 :=
  QuadraticNumberFields.Zsqrtd.lift (2 : ZMod 9) (by decide)

/-- Ring hom sending `√-5` to `-2` in `ZMod 9`. -/
noncomputable def phiMinus9 : R →+* ZMod 9 :=
  QuadraticNumberFields.Zsqrtd.lift ((-2 : ℤ) : ZMod 9) (by decide)
...
private theorem ramificationIdx_eq_one_of_le_and_not_sq
  ...
  · rw [QuadraticNumberFields.Zsqrtd.Ideal.map_span_int_singleton, pow_one]
    simpa using hP
  · intro h
    rw [QuadraticNumberFields.Zsqrtd.Ideal.map_span_int_singleton] at h
    exact hnot h
...
lemma not_span3_le_P3₁_sq : ¬(span ({(3 : R)} : Set R) : Ideal R) ≤ P3₁ ^ (1 + 1) := by
  ...
  exact QuadraticNumberFields.Zsqrtd.lift_intCast 2 _ (3 : ℤ)
...
lemma comap_P2 : Ideal.comap (algebraMap ℤ R) P2 = (span ({(2 : ℤ)} : Set ℤ) : Ideal ℤ) :=
  QuadraticNumberFields.Zsqrtd.Ideal.comap_span_p_one_plus_sqrtd 2 neg5_dvd_two

lemma comap_P3₁ : Ideal.comap (algebraMap ℤ R) P3₁ = (span ({(3 : ℤ)} : Set ℤ) : Ideal ℤ) :=
  QuadraticNumberFields.Zsqrtd.Ideal.comap_span_p_one_plus_sqrtd 3 neg5_dvd_three

lemma comap_P3₂ : Ideal.comap (algebraMap ℤ R) P3₂ = (span ({(3 : ℤ)} : Set ℤ) : Ideal ℤ) :=
  QuadraticNumberFields.Zsqrtd.Ideal.comap_span_p_one_minus_sqrtd 3 neg5_dvd_three

/-- The quotient by `P2` is canonically isomorphic to `ZMod 2`. -/
noncomputable def quotEquivP2 : (R ⧸ P2) ≃+* ZMod 2 :=
  QuadraticNumberFields.Zsqrtd.Ideal.quotEquivZModPNeg 2 neg5_dvd_two

/-- The quotient by `P3₁` is canonically isomorphic to `ZMod 3`. -/
noncomputable def quotEquivP3₁ : (R ⧸ P3₁) ≃+* ZMod 3 :=
  QuadraticNumberFields.Zsqrtd.Ideal.quotEquivZModPNeg 3 neg5_dvd_three

/-- The quotient by `P3₂` is canonically isomorphic to `ZMod 3`. -/
noncomputable def quotEquivP3₂ : (R ⧸ P3₂) ≃+* ZMod 3 :=
  QuadraticNumberFields.Zsqrtd.Ideal.quotEquivZModP 3 neg5_dvd_three
...
theorem ramificationIdx_P2 :
  ...
  rw [ramificationIdx_spec]
  · rw [QuadraticNumberFields.Zsqrtd.Ideal.map_span_int_singleton]
...
theorem inertiaDeg_P2 :
  ...
  have hfin := Algebra.finrank_eq_of_equiv_equiv
    (Int.quotientSpanNatEquivZMod 2) quotEquivP2 (by
      ext n
      simp [quotEquivP2, QuadraticNumberFields.Zsqrtd.Ideal.quotEquivZModPNeg,
        QuadraticNumberFields.Zsqrtd.Ideal.liftModPNeg, P2])
...
theorem inertiaDeg_P3₁ :
  ...
  have hfin := Algebra.finrank_eq_of_equiv_equiv
    (Int.quotientSpanNatEquivZMod 3) quotEquivP3₁ (by
      ext n
      simp [quotEquivP3₁, QuadraticNumberFields.Zsqrtd.Ideal.quotEquivZModPNeg,
        QuadraticNumberFields.Zsqrtd.Ideal.liftModPNeg, P3₁])
  exact_mod_cast hfin.trans (by simp [Module.finrank_self])
...
theorem inertiaDeg_P3₂ :
  ...
  have hfin := Algebra.finrank_eq_of_equiv_equiv
    (Int.quotientSpanNatEquivZMod 3) quotEquivP3₂ (by
      ext n
      simp [quotEquivP3₂, QuadraticNumberFields.Zsqrtd.Ideal.quotEquivZModP,
