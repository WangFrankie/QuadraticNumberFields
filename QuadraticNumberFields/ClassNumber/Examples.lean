/-
Copyright (c) 2026 Frankie Wang. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Frankie Wang
-/

import QuadraticNumberFields.ClassNumber.Minkowski
import QuadraticNumberFields.Families.Basic
import QuadraticNumberFields.RingOfIntegers.CommonInstances

/-!
# Class Number Examples

This file will contain concrete class-number computations for small quadratic
fields.
-/

namespace QuadraticNumberFields
namespace ClassNumber

/-- Placeholder predicate for completed example computations. -/
def HasClassNumberExample (d : ℤ) : Prop :=
  Families.IsAdmissibleParam d

end ClassNumber
end QuadraticNumberFields
