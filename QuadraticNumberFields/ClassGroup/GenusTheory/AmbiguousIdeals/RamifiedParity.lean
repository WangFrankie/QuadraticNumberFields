...
ambiguous-ideal argument. It packages parity vectors for ramified prime factors,
the corresponding ordinary and narrow class-group products, and the finite
parity homomorphism whose kernel is used by the exact-sequence layer.
-/
...
/-- The erased ramified-prime parity vector of a nonzero integral ideal, omitting
the distinguished ramified prime `p0`. -/
...
/-- The narrow ideal class of the distinguished prime above a ramified rational
prime. -/
...
/-- The erased ramified-prime ideal product attached to a `Fin 2` parity vector. -/
...
/-- The full ramified-prime ideal product attached to a `Fin 2` parity vector. -/
...
/-- Forgetting positivity sends a ramified prime narrow class to the ordinary
class of the same ramified prime ideal. -/
...
/-- The ordinary class of a ramified prime ideal has square one. -/
...
/-- Positive natural numbers are totally positive after embedding in a fraction
ring. -/
...
/-- A principal ideal generated by a totally positive element is trivial in the
narrow class group. -/
...
/-- Conjugation preserves normalized-factor multiplicities in an ambiguous
ideal. -/
...
/-- A normalized factor above a ramified rational prime is the distinguished
ramified prime ideal above that rational prime. -/
...
/-- Being ramified is equivalent to being one of the distinguished ramified
prime ideals. -/
...
/-- The ramified rational prime attached to a normalized factor known to be
ramified. -/
...
/-- A ramified normalized factor is the distinguished prime ideal over its chosen
...
/-- Finset form of `narrowClassGroup_mk0_eq_normalizedFactors_prod`, grouping
...
/-- The ramified normalized-factor contribution is the product over the
distinguished ramified prime ideals, counted by multiplicity. -/
...
/-- The narrow class of a ramified prime ideal has square one. -/
...
/-- Ordinary class-group product attached to an erased ramified parity vector. -/
...
/-- The ordinary class-group product attached to an erased ramified parity vector
has square one. -/
...
/-- Narrow class-group product attached to an erased ramified parity vector. -/
...
/-- Narrow class-group product attached to a full ramified parity vector. -/
...
/-- Taking the narrow class of an erased ramified parity ideal product gives the
corresponding erased ramified parity class product. -/
...
/-- Taking the narrow class of a full ramified parity ideal product gives the
corresponding full ramified parity class product. -/
...
/-- Forgetting positivity sends the erased ramified narrow-class product to the
ordinary ramified class product. -/
...
/-- The erased ramified narrow-class product has square one. -/
...
/-- The full ramified narrow-class product has square one. -/
...
/-- The full ramified parity homomorphism evaluates to the full ramified
narrow-class product. -/
...
/-- Kernel membership for the full ramified parity homomorphism is triviality of
the corresponding full ramified narrow-class product. -/
...
/-- A `Fin 2`-valued additive vector with a nonzero coordinate is nontrivial
after passing to `Multiplicative`. -/
...
/-- A nontrivial multiplicative `Fin 2`-valued vector has a nonzero coordinate. -/
...
/-- The full ramified parity narrow-class product of an ideal's parity vector is
the ramified-prime count product from its normalized factorization. -/
...
/-- Narrow ideal classes fixed by inversion, the group-theoretic target counted
by the ambiguous-ideal upper bound. -/
