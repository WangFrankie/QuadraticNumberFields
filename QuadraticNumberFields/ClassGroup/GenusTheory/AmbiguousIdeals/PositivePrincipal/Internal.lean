...
/-- A principal nonzero ideal has a fraction-field generator whose principal
fractional ideal is its `mk0`. -/
...
/-- Norm formula for the fraction-field image of `a + √d` in `Q(√d)`. -/
...
/-- If `d < a ^ 2`, then the algebraic integer `a + √d` is nonzero. -/
...
/-- If `d < a ^ 2`, then the norm of the fraction-field image of `a + √d`
is positive. -/
...
/-- If a full ramified parity product is the span of `a + √d` with `d < a ^ 2`,
then it has a fraction-field generator of positive `Q(√d)` norm. -/
...
/-- The rational prime `2` is ramified in `Q(√-1)`. -/
...
/-- The ramified prime ideal above `2` in `Q(√-1)` is principal. -/
...
/-- A ramified prime ideal has nonzero full ramified parity at its own rational
prime. -/
...
/-- Factorization-level class identity: an ambiguous ideal has the same narrow
class as the full ramified parity product selected by its parity vector. -/
