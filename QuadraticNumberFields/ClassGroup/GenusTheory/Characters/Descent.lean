...
private theorem rawGenusCharacter_eq_one_of_narrowMk0_eq_one
  ...
  obtain ⟨a, hapos, ha⟩ :=
    NarrowClassGroup.exists_isTotallyPositive_generator_of_mk0_eq_one hI_mk0
  apply Units.ext
...
noncomputable def genusCharacter (p : RamifiedPrimeIndex d) :
    Cl⁺(d) →* ℤˣ :=
  ((Con.ker (narrowMk0OnGenusCoprimeIdeals d)).lift
      (rawGenusCharacter d p) (by
        intro I J hIJ
        rw [Con.ker_rel] at hIJ ⊢
        exact rawGenusCharacter_eq_of_narrowMk0_eq d p I J hIJ)).comp
    (Con.quotientKerEquivOfSurjective
      (narrowMk0OnGenusCoprimeIdeals d)
      (narrowMk0OnGenusCoprimeIdeals_surjective d)).symm.toMonoidHom
...
theorem genusCharacter_apply_mk0
    ...
    genusCharacter d p (narrowMk0OnGenusCoprimeIdeals d I) =
      rawGenusCharacter d p I := by
  simp only [genusCharacter, MonoidHom.coe_comp, MulEquiv.coe_toMonoidHom,
    Function.comp_apply, Con.quotientKerEquivOfSurjective,
    Con.quotientKerEquivOfRightInverse]
  apply rawGenusCharacter_eq_of_narrowMk0_eq
  exact (narrowMk0OnGenusCoprimeIdeals_surjective d).hasRightInverse.choose_spec
    (narrowMk0OnGenusCoprimeIdeals d I)
