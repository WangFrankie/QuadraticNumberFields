...
theorem exists_ideal_in_class_of_norm_le_imaginary (hd : d < 0)
...
theorem exists_ideal_in_class_of_norm_le_real (hd : 0 < d)
    (C : ClassGroup (𝓞 (Qsqrtd (d : ℚ)))) :
    ∃ I : nonZeroDivisors (Ideal (𝓞 (Qsqrtd (d : ℚ)))),
      ClassGroup.mk0 I = C ∧
        (absNorm (I : Ideal (𝓞 (Qsqrtd (d : ℚ)))) : ℝ) ≤
...
theorem exists_ideal_in_class_of_norm_le (C : Cl(d)) :
    ∃ I : nonZeroDivisors (Ideal (𝓞 (Qsqrtd (d : ℚ)))),
      ClassGroup.mk0 I = C ∧
...
theorem classGroup_mem_closure_prime_ideals_le_minkowskiBound (C : Cl(d)) :
    C ∈ Subgroup.closure
      {C' : Cl(d) |
...
numbers. -/
theorem minkowskiBound_lt_of_neg (hd : d < 0) {n : ℕ}
