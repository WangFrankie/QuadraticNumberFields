...
/-- A nonzero prime ideal over an integral extension of `ℤ` lies above a
principal ideal generated by a natural prime. -/
private theorem exists_natPrime_comap_eq_span_of_primeIdeal
    {R : Type*} [CommRing R] [IsDomain R] [Algebra.IsIntegral ℤ R]
    {P : Ideal R} [P.IsPrime] (hP0 : P ≠ ⊥) :
    ∃ p : ℕ, p.Prime ∧
      P.comap (algebraMap ℤ R) = Ideal.span ({(p : ℤ)} : Set ℤ) := by
  let p := Ideal.absNorm (P.under ℤ)
  haveI : NeZero P := ⟨hP0⟩
  refine ⟨p, Nat.absNorm_under_prime P, ?_⟩
  rw [← Ideal.under_def]
  exact (Int.ideal_span_absNorm_eq_self (P.under ℤ)).symm

/-- The rational prime below a normalized Dedekind prime factor. -/
private theorem normalizedFactor_exists_natPrime_comap_eq_span
    {R : Type*} [CommRing R] [IsDomain R] [IsDedekindDomain R]
    [Algebra.IsIntegral ℤ R]
    (I : (Ideal R)⁰)
    (P : {P // P ∈ UniqueFactorizationMonoid.normalizedFactors (I : Ideal R)}) :
    ∃ p : ℕ, p.Prime ∧
      P.1.comap (algebraMap ℤ R) = Ideal.span ({(p : ℤ)} : Set ℤ) := by
  have hI0 : (I : Ideal R) ≠ ⊥ := by
    simpa [Ideal.zero_eq_bot] using mem_nonZeroDivisors_iff_ne_zero.mp I.2
  have hPdata := (Ideal.mem_normalizedFactors_iff hI0).mp P.2
  haveI : P.1.IsPrime := hPdata.1
  have hP0 : P.1 ≠ ⊥ := by
    intro hPbot
    exact hI0 (le_bot_iff.mp (by simpa [hPbot] using hPdata.2))
...
private theorem normalizedFactor_eq_ramifiedPrimeIdeal_of_isRamifiedIn
    (d : ℤ) [Fact (Squarefree d)] [Fact (d ≠ 1)]
    {I : (Ideal (NumberField.RingOfIntegers (Qsqrtd (d : ℚ))))⁰}
    (P : {P // P ∈ UniqueFactorizationMonoid.normalizedFactors
      (I : Ideal (NumberField.RingOfIntegers (Qsqrtd (d : ℚ))))})
      ...
      ∀ (P :
        ...
        ∃ p : ℕ, p.Prime ∧
          P.1.comap
            ...
            𝔭(p) := by
    intro P
    exact normalizedFactor_exists_natPrime_comap_eq_span J P
  have hfactor_narrow :
      ∀ (P :
          {P // P ∈ UniqueFactorizationMonoid.normalizedFactors
            (J : Ideal (NumberField.RingOfIntegers (Qsqrtd (d : ℚ))))})
