...
This imports the completed, sorry-free public library. Work-in-progress
developments are re-exported separately from:
...
See `docs/design/quadratic-field-architecture.md` for the design notes.
...
- Lean/mathlib/repl `v4.30.0-rc2`, as pinned by `lean-toolchain` and
  `lakefile.toml`
...
Use `lake exe cache get` after cloning or after dependency changes to download
mathlib build artifacts.

## Development Notes

- `QuadraticNumberFields.lean` should re-export only completed, sorry-free
  modules.
- `QuadraticNumberFields/Sketch.lean` is the holding area for unfinished modules
  and theorem skeletons.
- General-purpose facts intended for mathlib live under
  `QuadraticNumberFields/Mathlib/`, mirroring the intended mathlib path when
  practical.
